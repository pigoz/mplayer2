...
static int write_jpeg(screenshot_ctx *ctx, mp_image_t *image, char *filename)
{
    ...
    jpeg_set_quality(&cinfo, ctx->mpctx->opts.screenshot_quality, 1);
...
static char *talloc_strftime(void *talloc_ctx, const char *fmt, struct tm *tm)
{
    char buffer[20];
    if (strftime(buffer, sizeof(buffer), fmt, tm) == 0)
        buffer[0] = '\0';
    return talloc_strdup(talloc_ctx, buffer);
}

static char *talloc_stripext(void *talloc_ctx, const char *s)
{
...
static char *create_fname(screenshot_ctx *ctx, int *out_uses_frameno)
{
    ...
    *out_uses_frameno = 0;
    ...
    for (;;) {
        ...
        switch (*template++) {
            case 'n':
                res = talloc_asprintf_append(res, "%04d", ctx->frameno);
                *out_uses_frameno = 1;
                break;
            case 'f': {
                char *video_file = get_metadata(ctx->mpctx, META_NAME);
                if (video_file)
                    res = talloc_strdup_append(res, video_file);
            ...
            case 'F': {
                char *video_file = get_metadata(ctx->mpctx, META_NAME);
                if (video_file) {
                    char *sname = talloc_stripext(res, video_file);
                    res = talloc_strdup_append(res, sname);
                }
                talloc_free(video_file);
                break;
            }
            case 'p':
                res = talloc_strdup_append(res,
                        format_time(res, get_current_time(ctx->mpctx), false));
                break;
            case 'P':
                res = talloc_strdup_append(res,
                        format_time(res, get_current_time(ctx->mpctx), true));
                break;
            case 't': {
                char fmt = *template;
                if (!fmt)
                ...
                localtime_r(&t, &tm);
                char fmtstr[] = {'%', fmt, '\0'};
                char *str = talloc_strftime(res, fmtstr, &tm);
                res = talloc_asprintf_append(res, "%s", str);
                talloc_free(str);
                break;
            }
            case '%':
...
void screenshot_save(struct MPContext *mpctx, struct mp_image *image)
{
    ...
    if (filename) {
        ...
        if (!get_writer(ctx)->write(ctx, dst, filename))
