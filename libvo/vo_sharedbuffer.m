 ...
 * coordination with an external GUI using Cocoa's Distributed Objects.
 */
...
static void free_buffers(void)
{
    ...
    if (p->image_data) {
        if (munmap(p->image_data, image_bytes()) == -1)
            mp_msg(MSGT_VO, MSGL_FATAL,
                "[vo_sharedbuffer] uninit: munmap failed. Error: %s\n",
                strerror(errno));

        if (shm_unlink(p->buffer_name) == -1)
            mp_msg(MSGT_VO, MSGL_FATAL,
                "[vo_sharedbuffer] uninit: shm_unlink failed. Error: %s\n",
                strerror(errno));
    }
}
