 ...
 * to copy mp_image data to a shared buffer using mmap and to do simple coordination
 * with the GUIs using Distributed Objects.
 */
...
static int preinit(struct vo *vo, const char *arg)
{
    ...
    if (subopt_parse(arg, subopts) != 0) {
        mp_msg(MSGT_VO, MSGL_FATAL,
                ...
                "Example: mplayer -vo sharedbuffer:shared_buffer:buffer_name=mybuff\n"
                "\nOptions:\n"
                "  buffer_name=<name>\n"
...
static void free_buffers(void)
{
    ...
    if (p->image_data) {
        if (munmap(p->image_data, image_bytes()) == -1)
            mp_msg(MSGT_VO, MSGL_FATAL, "[vo_sharedbuffer] uninit: munmap failed. "
                                        "Error: %s\n", strerror(errno));

        if (shm_unlink(p->buffer_name) == -1)
            mp_msg(MSGT_VO, MSGL_FATAL, "[vo_sharedbuffer] uninit: shm_unlink failed. "
                                        "Error: %s\n", strerror(errno));
    }
}
                  ...
                  uint32_t format)
{
    ...
    if ([p->mposx_proxy conformsToProtocol:@protocol(MPlayerOSXVOProto)]) {
        ...
        [p->mposx_proto startWithWidth:p->image_width withHeight:p->image_height
                             withBytes:p->image_bytespp withAspect:d_width*100/d_height];
    } else {
