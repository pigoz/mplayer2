...
static void m_config_free_options(struct m_config *config, void *optstruct)
{
    ...
    for (copt = config->opts; copt; copt = copt->next) {
        ...
        if (copt->opt->type->flags & M_OPT_TYPE_DYNAMIC) {
            void *ptr = m_option_get_ptr(copt->opt, optstruct);
            if (ptr)
...
{
    m_config_free_options(config, config->optstruct);
    struct m_config_option *copt;
    for (copt = config->opts; copt; copt = copt->next) {
        if (copt->flags & M_CFG_OPT_ALIAS)
            continue;
